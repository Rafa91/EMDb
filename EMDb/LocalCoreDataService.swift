...
//  EMDb
//
//  Created by Rafael Navarro on 27/3/17.
...
class LocalCoreDataService{
    ...
    let remoteMovieService = RemoteiTunesService()
    ...
    func getTopMovies(localHandler: ([Movie]?) -> Void , remoteHandler: @escaping ([Movie]?)-> Void){
        ...
        remoteMovieService.getTopMovies(){ movies in
            if let movies = movies{
                ...
                //Delete unsync Movies
                
                self.removeOldNotFavoritesMovies()
    ...
    func removeOldNotFavoritesMovies(){
        let context = stack.persistentContainer.viewContext
        let request : NSFetchRequest<MovieManaged> = MovieManaged.fetchRequest()
        ...
        do {
            ...
            for managedMovie in fetchMovies{
    ...
    func getFavoriteMovies() -> [Movie]?{
        ...
        do{
            ...
            print("Error while gettin favorites")
            return nil
        }
